...
class Planifions extends Table
{

    protected static $table ="planifions";

    public static function addPlanifions($file,$titre)
    {
        $errors = [];
        ...
        if (empty($errors)) {
            ...
            //var_dump($config->get('pathToUp').'\planifions\\');
            //die();
            move_uploaded_file($image_temp, $config->get('pathToUp').'/planifions/' . $image_name);
            //var_dump($image_name);
            ...
            return self::call()->prepare($sql,[$image_name,$titre],null);
