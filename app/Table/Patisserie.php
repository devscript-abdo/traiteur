...
class Patisserie extends Table
{


    protected static $table ="patisserie";

    public static function addPatisserie($file,$type)
    {
        $errors = [];
        ...
        if (empty($errors)) {
            ...
            move_uploaded_file($image_temp, $config->get('pathToUp').'/patisserie/' . $image_name);
            //var_dump($image_name);
            // $img_path = $_SERVER['DOCUMENT_ROOT']. '\tr\img\apropos\\' .$image_name;
            // $sql = "INSERT INTO ".self::$table." (titre) VALUES (:photo1)";

            $sql = "INSERT INTO ".self::$table." SET image = ? ,type = ?";

            // $sql = "UPDATE apropos SET ".$photo." = ? WHERE id = 1";

            return self::call()->prepare($sql,[$image_name,$type],null);
